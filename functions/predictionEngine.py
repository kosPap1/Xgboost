...
def predictionEngine1(loadTrainDayTarget, loadTrainDayMinus1, loadTrainDayMinus7, loadTrainDayMinus365, loadTestDayTarget,
    ...
    MLPreg = MLPRegressor(hidden_layer_sizes=(24, 24), activation="relu", solver='adam', random_state=0,
    ...
    model = xgb.XGBRegressor(n_estimators=250, learning_rate=0.2, max_depth=3, base_score=0.9, booster='gbtree')
...
def predictionEngineCase2(loadTrainDayTarget, loadTrainDayMinus1, loadTrainDayMinus7, loadTrainDayMinus365, loadTestDayTarget,
    ...
    # running xgboost predictor
    model = xgb.XGBRegressor(n_estimators=300, learning_rate=0.1, max_depth=3, base_score=0.9, booster='dart')
    model.fit(trainSetX, trainSetY)
    ...
    mape = (mean_absolute_percentage_error(testSetY, h_prediction)) * 100
    maxError = metrics.max_error(testSetY, h_prediction)
    print('MAPE with mlp is ' + str(mape))
    mape2 = (mean_absolute_percentage_error(testSetY, y_predXGB)) * 100
    print('MAPE with xgb is ' + str(mape2))
    print('\n')
...
def predictionEngineCase3(loadTrainDayTarget, loadTrainDayMinus1, loadTrainDayMinus7, loadTrainDayMinus365,
        ...
        y_predXGB = scaler.inverse_transform(y_predXGB)
        h_prediction = scaler.inverse_transform(h_prediction)
        testSetY = scaler.inverse_transform(testSetY)
        ...
        print('Model score by Metrics.R2 for MLP equals to ' + str(metrics.r2_score(testSetY, h_prediction)))
        print('Model score by Metrics.R2 for XGB equals to ' + str(metrics.r2_score(testSetY, y_predXGB)))
        # print('Models max error is ' + str(maxError))

        print('\n')

        # Plots
        plt.plot(y_predXGB, label='XGB')
        plt.plot(testSetY, label='Actual Value')
        plt.plot(h_prediction, label=' MLP')
        #
        plt.title(str(filename))
        plt.legend()
        plt.show()
